...
The selection prompt marks where educational intent enters model procedure. A fixed-ratio cloze test hides its assumptions inside arithmetic. Cloze Reader writes the selection assumptions out in natural language, letting the player infer from the prompt and letting the critic inspect the heuristics that shape each blank. Bommasani et al. (2021) describe foundation model abilities as outcomes of training, and Cloze Reader leaves training opacity intact while letting readers inspect the prompt-level decisions that happen before opacity takes over.
