...
I also use the progression to invert the model's relation to direction. A masked model holds both sides of a blank at once because its access to context is fixed in the architecture, while a player meets the passage in time. The reader moves toward the blank from the left and accumulates the expectation Oller described, reaching the gap with only the preceding words in hand, then earns the right-hand context by reading past the blank and returning. The model receives at once the context that the player assembles in sequence. Blank count sharpens the difference. At one blank the surround stays intact and the task sits close to a single masked position in full context, but as later levels add a second and third blank, the context the reader needs is itself partly removed and the blanks begin to constrain one another. The independence a masked model assumes across masked positions (Yang et al. 2019) is exactly what a player cannot afford across several blanks, since each committed word rewrites the context for the next and forces the reader to resolve the blanks in some order. As the game advances, the asymmetry widens rather than narrows, because the more the passage is occluded, the more the reader must perform the joint, ordered inference that the model factors apart.
