...
Machine learning researchers, working from different disciplinary starting points, recognized the same core premise, namely, that asking a system to predict a missing word from surrounding context tests whether it has internalized how language works.
...
The genealogy begins with two accounts of how language carries meaning, accounts that NLP researchers would eventually merge into a single training objective. Harris (1954) set meaning aside entirely, arguing that phonemes, morphemes, and words could be identified through their co-occurrence patterns alone. Elements appearing in the same environments belong to the same grammatical class, and the restrictions on how those classes combine generate the grammar. Firth (1957) took that structural observation and redirected it toward semantics. In "Modes of Meaning," he proposed that words carry content through their habitual collocations, or the words they characteristically appear alongside — a claim he compressed into the formula "you shall know a word by the company it keeps." For Firth, *time*'s tendency to collocate with *saved*, *spent*, or *wasted* was itself semantic information, since nothing internal to the word determines those pairings.

Mikolov, Peters, and Devlin each inherited both accounts and merged them into a single computable question. Given enough text, can a model learn what words mean by observing what they appear near? Mikolov showed that it could. The word2vec framework trained shallow neural networks to predict a word's surrounding context from the word itself (skip-gram) or to predict the word from its context (CBOW), producing vector representations in which words sharing distributional contexts clustered together in vector space. The limitation word2vec exposed was context-independence. Each word received a single vector regardless of how it appeared in any given sentence, treating *bank* identically whether it preceded *account* or followed *river*. Peters responded by training two LSTM language models in parallel, one left-to-right and one right-to-left, combining their outputs at each word position to produce representations shaped by the specific sentence the word appeared in. Devlin extended that logic, training a Transformer encoder to predict randomly masked tokens from the full bidirectional context jointly in every layer rather than merging separately trained directional passes at the output. Devlin called this objective masked language modeling and drew the analogy to the cloze task directly, having changed the scale and the architecture but not the task itself.

Cloze Reader ([reader.inference-arcade.com](https://reader.inference-arcade.com)), a browser-based educational game developed through the CUNY AI Lab and the Inference Arcade initiative, preserves the cloze prompt as a human-facing task while delegating word removal to a language model guided by heuristic instructions. That design holds two things in view at once: the continuity of the cloze premise across seventy years of research, and the divergence in what removal reveals about the system performing it.
...
Cloze testing and masked language modeling both descend from the distributional hypothesis, the claim Harris (1954) formalized and Firth (1957) made memorable, that words occurring in similar contexts carry similar meanings. The contextual prediction insight that Taylor applied to human readers is the same insight that Mikolov, Peters, and Devlin each formalized as a training objective, and that Cloze Reader returns to human players as a game.

Mikolov showed that word embeddings trained on shallow prediction objectives captured semantic and syntactic regularities at scale. The limitation of such representations was context-independence. Each word received a single vector regardless of its usage in a specific sentence. Peters responded with ELMo (Embeddings from Language Models), which generated contextualized representations via bidirectional LSTM language modeling with forward and backward passes trained separately and then concatenated. ELMo's representations showed that the prediction task required context-sensitive encoding, a precondition BERT extended by making the masking objective joint across all token positions simultaneously.

BERT (Devlin et al. 2019) extended ELMo's bidirectionality by training on masked tokens within a single joint model. Where ELMo concatenated two separately trained directional passes, BERT's transformer encoder let every token position attend to all others simultaneously. Masking was feasible as a self-supervised objective because unannotated text already contained the supervisory signal, and the scale of available text made large-parameter training viable (Liu et al. 2021; Bommasani et al. 2021). Devlin's results established masking as the dominant pretraining objective.
...
I chose Project Gutenberg as the source archive because these books already occupy a determinate place in the history of language model training. Large pretraining corpora treat them as standard source material, breaking them into tokenized sequences before any player encounters them in the game. That prior ingestion is the condition Cloze Reader starts from, not a background fact. The passages in the app come from Project Gutenberg's public-domain holdings (Project Gutenberg 2026; Cloze Reader 2026a), and those holdings recur in major pretraining datasets. The Pile, an 800-gigabyte corpus assembled by EleutherAI, includes Gutenberg texts among its source collections (Gao et al. 2020), while empirical work on memorization has shown that models trained on such corpora can reproduce verbatim passages from these books under targeted prompting conditions (Carlini et al. 2021).

These books have already passed through an extractive pipeline that converts literary works into model weights. Their narrative forms are broken down into token distributions, detached from authorship and historical situation, then returned in model outputs without acknowledgment. Gitelman and Jackson (2013) describe this broader conversion of situated texts into data. Cloze Reader takes that conversion as its critical artifact, since the game begins after the archive has already been ingested and asks the player to confront a passage that has survived the process only in transformed form.

Against this flattening, Cloze Reader restores the passage to the surface as an object the reader must work through rather than pass beyond. The language model becomes one layer in a larger procedure, not the whole of it, and its outputs become legible only when a human reader confronts the same passage the model encountered in transformed form.
...
Both histories share a structural premise, that contextual prediction is a meaningful index of linguistic competence. Taylor operationalized that premise as an assessment instrument. BERT's developers operationalized it as a training objective. The two applications differ in what they treat as the system under examination — human readers in one case, model weights in the other — and in what the procedure is meant to produce, a measurement of existing capacity or an induction of new representations.

Cloze Reader addresses this tension structurally. I divided the cloze task between model and player so that selection and interpretation stay separate. Masked language modeling does not require that separation. The training objective treats selecting where to mask and learning from the resulting prediction as a single operation, because gradient descent over token distributions does not need to distinguish them. That division is what I staged rather than resolved. Taylor's procedure assessed comprehension as a holistic capacity. BERT's objective optimized prediction as a training signal. Cloze Reader makes those two operations visible side by side through play.
...
*Draft v45 · Updated 2026-03-30 EDT · Continuity and Asymmetry: contrastive simplification replaced with mechanistic account; colon removed from convergence paragraph*
