...
NLP researchers inherited both accounts and merged them into a single computable question: given enough text, can a model learn what words mean by observing what they appear near? Mikolov et al. (2013) showed that it could. Their word2vec framework trained shallow neural networks to predict a word's surrounding context from the word itself (skip-gram) or to predict the word from its context (CBOW), producing vector representations in which words sharing distributional contexts ended up near each other in vector space. The limitation this revealed was context-independence: each word received a single vector regardless of how it appeared in any given sentence, treating *bank* identically whether it appeared before *account* or after *river*. Peters et al. (2018) addressed this by training two LSTM language models in parallel, one left-to-right and one right-to-left, combining their outputs at each word position to produce representations shaped by the specific sentence the word appeared in. Devlin et al. (2019) carried that logic further, training a Transformer encoder to predict randomly masked tokens from the full bidirectional context jointly in every layer rather than merging separately trained directional passes at the output. Devlin et al. called this objective masked language modeling and drew the analogy to the cloze task directly, having updated the scale and the architecture but not the task itself.
