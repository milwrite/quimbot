...
**Cloze Reader makes this gap concrete.** The application uses Gemma-3 — a language model trained through the masked-prediction paradigm BERT established — to select which words to remove from a passage. The model picks words with high contextual salience, which means words its training made highly predictable. A player then tries to restore them. The model chose the blank because prediction was easy; the player struggles because comprehension is hard. The same word, the same blank, two entirely different cognitive situations. **Cloze Reader turns that asymmetry into observable data: every blank where Gemma-3 predicted confidently and the player hesitated is a recorded instance of the gap between statistical pattern-matching and situated human comprehension, and this paper uses those instances to argue that the two processes, despite sharing a surface form, diverge at the level of what counts as understanding.**
