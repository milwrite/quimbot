...
**Cloze Reader makes this gap concrete.** The application uses Gemma-3 — a language model trained through the masked-prediction paradigm BERT established — to select which words to remove from a passage. The model picks words with high contextual salience, which means words its training made highly predictable. A player then tries to restore them. The model chose the blank because prediction was easy; the player struggles because comprehension is hard. The same word, the same blank, two entirely different cognitive situations. **Cloze Reader puts a language model and a human reader on opposite sides of the same blank for the first time, Gemma-3 selecting deletions by contextual predictability and the player restoring them by comprehension, and in doing so it makes visible something the shared notation has obscured since Taylor. The cloze procedure did not migrate from psycholinguistics into machine learning intact. It split into two practices that use the same surface form to do incompatible work, one measuring whether a reader already understands a passage and the other training a model that has never understood anything, and the fact that both call the result "fill in the blank" is precisely the confusion this paper means to open up.**
