...
**Cloze Reader makes this gap concrete.** The application uses Gemma-3 — a language model trained through the masked-prediction paradigm BERT established — to select which words to remove from a passage. The model picks words with high contextual salience, which means words its training made highly predictable. A player then tries to restore them. The model chose the blank because prediction was easy; the player struggles because comprehension is hard. The same word, the same blank, two entirely different cognitive situations. **Cloze Reader places these two histories inside the same interaction, a language model descended from BERT's masked-prediction paradigm choosing which words to remove and a human reader descended from Taylor's tradition of contextual inference trying to put them back, and the artifact does not resolve the tension between them but holds it open. The blank that Taylor designed to reveal comprehension and the blank that Devlin repurposed to train prediction sit in the same position on the screen and ask entirely different things of the person and the process that encounter them, and whether that difference can survive the convergence of form is a question this paper raises but does not pretend to close.**
