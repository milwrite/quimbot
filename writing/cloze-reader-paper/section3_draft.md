...
**Cloze Reader makes this gap concrete.** The application uses Gemma-3 — a language model trained through the masked-prediction paradigm BERT established — to select which words to remove from a passage. The model picks words with high contextual salience, which means words its training made highly predictable. A player then tries to restore them. The model chose the blank because prediction was easy; the player struggles because comprehension is hard. The same word, the same blank, two entirely different cognitive situations. **Cloze Reader divides the task along the fault line the CLOTH results exposed. The model selects deletions, locating where context makes a word statistically recoverable, and a reader attempts restoration, drawing on the author's register and the arc of the passage and the accumulated distance between nineteenth-century prose and contemporary intuition that no training corpus encodes as such. That division separates selection from interpretation and gives the asymmetry a concrete form. Taylor built the cloze procedure to measure whether readers already understood a passage. Masked language modeling repurposed the same structure to train models that had never understood anything. Cloze Reader returns the procedure to human readers with the model's selection logic still visible in the architecture, and the gap between what the model found trivially predictable and what the player finds genuinely difficult is the space where seventy years of divergent purpose become legible as a single interaction.**
