...
**Cloze Reader makes this gap concrete.** The application uses Gemma-3 — a language model trained through the masked-prediction paradigm BERT established — to select which words to remove from a passage. The model picks words with high contextual salience, which means words its training made highly predictable. A player then tries to restore them. The model chose the blank because prediction was easy; the player struggles because comprehension is hard. The same word, the same blank, two entirely different cognitive situations. **When Gemma-3 selects a word for deletion, it selects the word it could most easily predict from context. When a player encounters that same blank in a passage from Melville or Douglass, restoring it requires not prediction but comprehension: knowledge of the author's period, register, and argument, plus the ability to hold the surrounding passage in mind as a coherent whole. Cloze Reader logs both sides of that transaction. The resulting data — which blanks the model found trivial and the player found hard, which blanks both solved easily, which neither could recover — gives this paper its empirical basis for distinguishing two processes that the cloze format has, since Taylor, made look identical.**
