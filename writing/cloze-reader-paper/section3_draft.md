...
**Cloze Reader divides the task along the line the CLOTH results drew between statistical plausibility and contextual comprehension.** The application uses Gemma-3, a language model trained through the masked-prediction paradigm BERT established, to select which words to remove from a passage, locating where context makes a word statistically recoverable. A player then attempts restoration, drawing on the author's register and the arc of the passage and the accumulated distance between nineteenth-century prose and contemporary intuition that no training corpus encodes as such. That division separates selection from interpretation and gives the asymmetry a form worth examining. Taylor built the cloze procedure to measure whether readers already understood a passage. Masked language modeling repurposed the same structure to train models that had never understood anything. Cloze Reader returns the procedure to human readers with the model's selection logic still visible in the architecture, and the gap between what the model found trivially predictable and what the player finds genuinely difficult opens a space where seventy years of divergent purpose can be read as a single interaction.
