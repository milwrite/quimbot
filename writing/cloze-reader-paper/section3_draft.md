...
**Cloze Reader makes this gap concrete.** The application uses Gemma-3 — a language model trained through the masked-prediction paradigm BERT established — to select which words to remove from a passage. The model picks words with high contextual salience, which means words its training made highly predictable. A player then tries to restore them. The model chose the blank because prediction was easy; the player struggles because comprehension is hard. The same word, the same blank, two entirely different cognitive situations. **The blank in a Melville passage that Taylor would have used to test a reader's grasp of nineteenth-century prose and the blank that BERT learned to fill by minimizing cross-entropy loss across a billion tokens appear, inside Cloze Reader, as the same gap on the same screen, and the application stages what the CLOTH results only measured at a distance. Gemma-3 selects the deletion because the word was predictable from local context, the kind of statistical regularity that masked language modeling was built to exploit, and the player encounters that deletion as a problem of comprehension that draws on everything the model never had access to, the author's period and register, the arc of the passage, the felt difference between a word that fits and a word that merely could. The cloze procedure did not survive its migration from psycholinguistics to machine learning as one practice. It became two, and Cloze Reader is an argument, built as a playable artifact, that the convergence of notation has hidden a divergence of purpose deep enough that calling both sides "fill in the blank" may be the most productive confusion in contemporary language technology.**
