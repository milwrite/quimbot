...
**Cloze Reader makes this gap concrete.** The application uses Gemma-3 — a language model trained through the masked-prediction paradigm BERT established — to select which words to remove from a passage. The model picks words with high contextual salience, which means words its training made highly predictable. A player then tries to restore them. The model chose the blank because prediction was easy; the player struggles because comprehension is hard. The same word, the same blank, two entirely different cognitive situations. **Cloze Reader reconnects these two histories by placing them inside the same interaction. A language model descended from BERT's masked-prediction paradigm chooses which words to remove from a passage; a human reader descended from Taylor's tradition of contextual inference tries to put them back. The artifact holds both processes in view simultaneously, not as an experiment that measures one against the other, but as a critical argument rendered in the form of a digital object: the blank that Taylor designed to reveal comprehension and the blank that Devlin repurposed to train prediction occupy the same position on the screen, and the distance between what each asks of its subject is the distance this paper means to hold open.**
